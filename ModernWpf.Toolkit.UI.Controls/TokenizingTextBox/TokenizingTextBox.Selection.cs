...
namespace ModernWpf.Toolkit.UI.Controls
{
    ...
    public partial class TokenizingTextBox
    {
        ...
        private bool MoveFocusAndSelection(MoveDirection direction)
        {
            ...
            {
                var currentItem = ItemContainerGenerator.ItemFromContainer(currentContainerItem);
                ...
                {
                    var newItem = ItemContainerGenerator.ContainerFromIndex(index) as TokenizingTextBoxItem;
        ...
        private void ClearAllTextSelections(TokenizingTextBoxItem ignoreItem)
        {
            ...
            foreach (var item in Items)
            {
                if (item is ITokenStringContainer)
                {
                    var container = ItemContainerGenerator.ContainerFromItem(item) as TokenizingTextBoxItem;
        ...
        private bool SelectNewItem(TokenizingTextBoxItem item, int increment, Func<int, bool> testFunc)
        {
            ...
            var currentIndex = ItemContainerGenerator.IndexFromContainer(item);
        ...
        internal async Task RemoveAllSelectedTokens()
        {
            ...
            {
                var container = ItemContainerGenerator.ContainerFromItem(SelectedItems[0]) as TokenizingTextBoxItem;

                if (ItemContainerGenerator.IndexFromContainer(container) != Items.Count - 1)
                {
